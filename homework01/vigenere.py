...
def decrypt_vigenere(ciphertext: str, keyword: str) -> str:
    ...
    for i in range(0, len(ciphertext)):
        if keyword[i % len(keyword)] in upEng:
            shift += upEng[25 - upEng.find(keyword[i % len(keyword)])]
        else:
            if keyword[i % len(keyword)] in lowEng:
                shift += lowEng[25 - lowEng.find(keyword[i % len(keyword)])]
            else:
                if keyword[i % len(keyword)] in upRus:
                    shift += upRus[32 - upRus.find(keyword[i % len(keyword)])]
                else:
                    if keyword[i % len(keyword)] in lowRus:
                        shift += lowRus[32 - lowRus.find(keyword[i % len(keyword)])]
