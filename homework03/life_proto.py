...
class GameOfLife:
    ...
    def create_grid(self, randomize: bool = False) -> Grid:
        if randomize:
            return [[random.randint(0,1) for i in range(0, self.cell_height)] for j in range(0, self.cell_width)]
        else:
    ...
    def get_neighbours(self, cell: Cell) -> Cells:
        i,j=cell
        nears = ( (i,j-1), (i-1,j), (i,j+1), (i+1,j), (i-1,j-1), (i-1,j+1), (i+1,j-1), (i+1,j+1))
        poss=[]
        for a,b in nears:
    ...
    def get_next_generation(self) -> Grid:
        ...
        for i in range(0,len(self.grid)):
            for j in range(0,len(self.grid[0])):
                ...
                if (self.grid[i][j]==1 and (n == 2 or n == 3)) or (self.grid[i][j]==0 and n==3):
                    grid[i][j] = 1
        return grid
